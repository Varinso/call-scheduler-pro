...
export function SlackWebhookSettings() {
  const { user } = useAuth();
  ...
  async function handleSave() {
    ...
    const { error } = await supabase
      ...
      .upsert(
        {
          integration_name: "slack_webhook",
    ...
    if (error) {
      ...
      toast({ title: "Slack webhook saved", description: enabled ? "Notifications are active." : "Webhook saved but disabled." });
    }
    setSaving(false);
  }
  ...
  async function handleTest() {
    ...
    try {
      ...
      toast({ title: "Test sent!", description: "Check your Slack channel for the notification." });
    } catch (err) {
      toast({
        title: "Test failed",
    ...
    <Card className="border-border/50 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <CardDescription className="text-xs mt-0.5">
                Post meeting details to a Slack channel automatically
              </CardDescription>
            </div>
          </div>
          <Badge
      ...
      <CardContent className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="slack-webhook-url" className="text-xs font-medium">
            Webhook URL
          </Label>
          <Input
          ...
          <p className="text-xs text-muted-foreground">
            Create an Incoming Webhook in your Slack workspace: Apps → Incoming Webhooks → Add to Slack → Choose channel
          </p>
        </div>
        ...
        <div className="flex items-center justify-between rounded-lg border border-border/50 p-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Post to Slack when meetings are scheduled</p>
          </div>
